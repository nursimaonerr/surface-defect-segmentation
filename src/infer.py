...
def predict_overlay(model, image_rgb: np.ndarray, device: torch.device, threshold: float = 0.5) -> np.ndarray:
    # CLI (main) ve Gradio arayuzu (app.py) ayni tahmin mantigini burada paylasir
    ...
    masks = masks[:, :h, :w]
    return overlay_masks(image_rgb, masks)
...
def main():
    ...
    overlay = predict_overlay(model, image_rgb, device, args.threshold)
